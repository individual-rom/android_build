...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^dokdo_") ; then
       CM_BUILD=$(echo -n $1 | sed -e 's/^dokdo_//g')
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/dokdo/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function eat()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.dokdo.device=$CM_BUILD");
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/dokdo/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
